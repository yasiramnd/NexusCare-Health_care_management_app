...
export default function PriorityOrders({ onBack }) {
  ...
  return (
    <div className="po-page" style={{ minHeight: "auto", background: "transparent", padding: 0, maxWidth: "100%" }}>
      {/* HEADER */}
      <div className="po-header" style={{ padding: "0 0 20px" }}>
        <div>
          <h1 className="po-title">Priority Orders</h1>
          <p style={{ color: "var(--p-text-dim)", fontSize: "14px", marginTop: "4px" }}>Action required immediately for urgent prescriptions.</p>
        </div>
      </div>

      {/* ALERT BANNER */}
      {urgentCount > 0 && (
        <div className="po-alert-banner" style={{ margin: "0 0 20px" }}>
      ...
      {urgentCount === 0 && (
        <div className="po-alert-banner success" style={{ margin: "0 0 20px" }}>
      ...
      {/* ORDERS LIST */}
      <div className="po-list" style={{ padding: 0, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "20px" }}>
        {orders.map(order => {
